...
(警告のみ) を壊さずに追加できた)。
**P3.6** LSP `textDocument/hover` / `textDocument/definition`: 組込関数
...
- **LSP サーバ**: `src/lsp_main.rs` に手書き JSON-RPC framing + diagnostics 配信 (`textDocument/publishDiagnostics`) + 簡易 `textDocument/hover` /
  ...
  **非スコープ対応**実装 — lambda パラメータ等がたまたま同名のトップレベル `def` と被ると、そちらの情報/位置を返してしまう
  既知の限界がある (本格的なスコープ解決には束縛変数を追跡する AST 走査が必要)。組込関数への `definition` はジャンプ先の
  seki ソースが無いので常に `null`。completion / 型表示は未対応 (`tower-lsp` への移行はしていない — 依存ゼロ方針のため手書きのまま)。
