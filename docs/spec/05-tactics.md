...
- **`if` の場合分け**: ゴール側だけでなく**仮定 (hypothesis) 側**に埋め込まれた
  ...
  条件が `and` / `or` / `not` の組合せのときは**先頭の原子条件**で分ける
  (`if (r >= 0 and r <= 1/2) then A else B` は `r >= 0` で分け、真の枝は
  `if r <= 1/2 then A else B`、偽の枝は `B` になる)。以前は連言のまま 1 つの
  仮定として積んでいたため、then 枝で `r >= 0` が使えず、else 枝には
  `not (...)` という選言しか残らなかった。分け方はタクティクとカーネルで
  共通 (`rewrite::first_if_condition` / `collapse_if_cond`)。
