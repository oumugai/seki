...
| `Approximate` | 浮動小数点で決めた — **偽を通しうる** (§6.0.13, §6.0.14) | `[approximate — floating point]` |
...
**続き (0.11.x)**: 上の「`0.1` ではなく `1.0/10.0` と書け」という注意は
**不要になりました**。小数リテラルは書かれたとおりの 10 進数として読みます
— `0.1` は 1/10、`0.6` は 3/5 です。

これは桁あふれ対策ではなく、**意味の問題**でした。二進の厳密値を使うと
`0.6` は 3/5 よりわずかに小さいので、

```seki
theorem t : forall a in Real, a >= 0.6 => (a * 50.0) >= 30.0 := by algebra
```

が**偽**になります。double については正しく、書かれた主張については
間違っています。副作用として `0.1 * 0.2 * 0.3` は 3/500 になり、
もう桁あふれしません。

### 6.0.13 `Real` は ℝ、評価器は `f64` (0.11.x で修正)
...
両方とも `kernel-checked from primitives` と報告されていました。
定理証明系にとって最悪の状態です。

原因は `Real` に 2 つの読みがあったこと。`by algebra` は ℝ として、
評価器は `f64` として読んでいました。どちらか一方が言語の意味です。

**決定: `Real` は ℝ。`f64` はその近似**。したがって:

1. カーネルは実数の比較を**厳密に**決める (有理数の範囲内なら)。
   上の `notp` は**拒否**されます。
2. 有理数の範囲外 (`sqrt`、`exp`、関数呼び出しの奥) では、評価器の答えを
   **信頼する**が検査したとは言わない。新しい信頼水準 `Approximate` が
   これを表します。
...
def b := 0.2
def c := 0.3
theorem viaDefs : (a + b) != c := by eval   -- [approximate — floating point]
```

構文的にリテラルを探しても命題 `(a + b) != c` には 1 つも現れないので、
**評価器が「実数に触れたか」を記録する** (`EvalCtx::touched_real`)
方式にしました。

**影響**: コーパスの 163 件が `sound` から `approximate` に変わりました。
内訳を見るとほぼすべてが数値計算の許容範囲チェックです:

```
|integSimpson 100 f_sq 0 1 - 1/3| < 0.000001
|sinTaylor 20 (pi/4) - sqrt2_over_2| < 0.0000001
```

これらは **double についての主張**であって、実数についての証明では
ありません。以前は「primitives から再確立」と報告していましたが、
それは過大な主張でした。`--strict` は `Approximate` を拒否します。

### 6.0.14 区間演算 — 163 件のうち 141 件は実は証明できた

「浮動小数点で計算しただけ」と言うのは正直ですが、**大雑把**でもあります。
上の主張は実数として**真**であり、それを捨てる必要はありません。
足りなかったのは、丸めた 1 個の数ではなく**保証された囲い**で計算する
手段でした。

`src/interval.rs` の `Interval` は厳密有理数の対 `[lo, hi]` で、真の値を
必ず含みます。演算は**外側に**丸めるので囲いは広がることしかなく、
狭まって嘘をつくことはありません。比較は囲い全体が決めたときだけ
`Some(true)` / `Some(false)` を返し、決まらなければ `None` — カーネルは
それを「判定なし」と読み、決して「成立」とは読みません。

カーネルは `by eval` のゴールに対して、まず厳密な有理数で、次に**区間で**、
最後に浮動小数点で判定します。区間で `true` が出れば `sound` です。
...
- **分母を 10¹² で打ち切る**。厳密有理数は厳密ですが `a/b + c/d` の分母は
  `b·d` まで増えるので、数百回の演算で i128 をあふれます。外側に丸めて
  分母を抑えると、1 演算あたり約 1e-12 広がる代わりに数が有界になります。
  外側に広げるのは常に健全です。
- **`sqrt` は二乗して検査する**。浮動小数点の見積もりから出発しますが、
  各端点は `l² <= x <= h²` を**確かめて**から採用します。見積もりが
  外れても反復回数が増えるだけです。
- **`if` の両枝の包(hull)を取る**。条件が囲いで決まらないとき —
  `absR x` で `x` が 0 をまたぐとき、つまり許容誤差チェックのほぼ全部 —
  両枝を評価して包を取ります。真の値はどちらかの枝にあるので健全です。
- **定義はソースから再評価する**。`def pi := 3.141592653589793` は宣言時に
  `f64` として評価済みで、その double を読み戻して厳密扱いするのは
  区間演算が防ごうとしている混同そのものです。式を保存して区間モードで
  評価し直します。ただし**副作用のある定義は除外**します —
  `def counter := mkRef 0` を二度評価すると別のセルになり、
  カーネルがプログラムの書き込んでいないセルを読むことになります。
- **丸めた `f64` は区間演算に入れない**。囲いにできない値が混ざったら
  判定を返しません。安全側に倒す方向です。

#### ソース側の区間値 (`interval lo hi`)

区間演算をカーネル内部に閉じ込めておくと、証明できるのは**具体的な
数値計算**に限られます。`interval lo hi` は「lo と hi の間のすべての実数」
を表す値で、これを書けると主張が**集合全体**についてのものになります:

```seki
def startup := interval 15.0 30.0
theorem never_overheats : evolve 10 startup <= 25.0 := by eval
```

比較は範囲全体が決めたときだけ答えます。境界をまたぐ場合は
「成り立つことも成り立たないことも示していない」という証明の失敗です
— これが健全性の要で、「決まらない」を「成立」と読むことは決してありません。

普通の (f64 の) 評価でも区間の値は区間として運ばれます。カーネルは
どのみちソースから評価し直し、裸の `f64` が区間演算に入ることを拒否
するので、実行側の扱いに健全性は依存しません。

#### 超越関数の囲い (exp / ln / sin / cos)

seki の TCB が数値解析をするのはここだけなので、**信頼すべきものが
「剰余項の評価」だけになる**ように書いてあります。級数そのものは上の
区間演算で評価する (すでに健全) ので、読み手が検査すべきは Lagrange の
剰余の形だけです:
...
- **sin / cos** — 導関数はすべて 1 以下なので、剰余は次の項の大きさそのもの。
  `[-1, 1]` との交差は常に正しいので最後にかけて締める。
  非単調なので**区間のまま級数を評価**する — 範囲の中に極値があっても
  取りこぼさず、過大評価になるだけ。
- **exp** — `|u| <= 1/2` になるまで半分にしてから級数、最後に二乗で戻す。
  その範囲で `exp(ξ) < 2` なので剰余は次の項の 2 倍以下。単調なので
  区間の像は端点の像。
- **ln** — `x = m·2^k` (m ∈ [1,2)) に分解。2 での乗除は厳密なので
  この分解は何も失わない。`ln m = 2·atanh(z)`、`z = (m-1)/(m+1) < 1/3` で
  剰余は等比的に抑えられる。`ln 2` は検証済みの有理数の囲いを直接置く。

**境界が保証されている範囲の外は拒否します** — 推定しません。
`sin`/`cos` は `|x| <= 8`、`exp` は半減 16 回まで。範囲外のゴールは
`Approximate` のままで、それが正しく、間違いようのない答えです。
`tan`/`atan`/`asin`/`acos`/双曲線関数は剰余の評価を書いていないので
同様に拒否します。

剰余の上界自体も**外側に丸めた区間**で計算します。厳密な有理数で
`|x|⁴¹/41!` を作ると分母があふれて poison になるからで、
上端を取れば上界であることは保たれます。

**結果**: `approximate` は 163 件から **8 件**に減りました。残る 8 件はすべて**反復アルゴリズムにおける区間の依存性**です。
Newton 法の `x - (x³-8)/(3x²)` は同じ変数が複数回現れるので、素朴な
区間演算では実数の反復のように縮まず、逆に広がります (wrapping effect)。
`cbrt8_approx` の囲いは ±1.17e-4 で、要求されている 1e-7 に届きません。

これは区間演算のよく知られた限界であってバグではありません。埋めるには
区間 Newton 法や平均値形式が要り、TCB がさらに増えます。
