...
`interval 8000.0 12000.0` は関数呼び出しで、包絡線を書く言語としては
素朴でした。3 つ足しています。

**記法**。仕様書は「中心 ± 許容差」で書かれるので、seki もそう読みます。

```seki
def rBand := 8000.0 .. 12000.0
def startup := 22.5 +- 7.5
```

どちらもパーサで `interval lo hi` に展開されます。新しい AST ノードを
作らないので、カーネルも型検査もタクティクも今までどおりのものしか見ず、
記法のコストがこの関数の外に出ません。結合は算術より緩く比較より強い
ので `1.0 + 2.0 .. 5.0` は 3 から 5 の帯です。

**内省**。`lo` / `hi` / `width` / `mid`。ただの数は幅ゼロの帯なので
そちらにも使えます。
...
`width` が効くのは、**囲いが広がっていないことを主張できる**ことです。
これが無いと、広がった囲いの症状は「決まらない主張」だけで、どれだけ
外れていたのかを言う手段がありません。

**診断**。決まらなかったときに**幅と理由**を出します。
...
区間が発散して演算自体が成立しなくなった場合も同じ扱いにしました
(以前は実行時エラーとして出ていました)。

途中で見つけたバグ: 囲いと数の `==` が、カーネルの中でしか三値で
扱われていませんでした。そのため `width x == 2.0` が通常の評価では
**構造的に不一致**と判定されていました。


### 超越関数の囲い — approximate 22 件 → 8 件

`exp` / `ln` / `sin` / `cos` に厳密な囲いを実装しました。TCB が数値解析を
するのはここだけなので、**信頼すべきものが「剰余項の評価」だけになる**
書き方にしてあります。級数は既存の区間演算で評価するので、読み手が
検査するのは Lagrange の剰余の形だけです。

| 関数 | 剰余の根拠 | 範囲 |
|---|---|---|
| `sin` / `cos` | 導関数がすべて 1 以下 → 次の項の大きさ | `\|x\| <= 8` |
| `exp` | `\|u\| <= 1/2` に半減してから `exp(ξ) < 2` | 半減 16 回まで |
| `ln` | `x = m·2^k` に分解、`2·atanh(z)` で `z < 1/3` | `x > 0` |

- `sin`/`cos` は非単調なので**区間のまま級数を評価**します。範囲の中に
  極値があっても取りこぼさず、過大評価になるだけです
  (`[1.4, 1.8]` の `sin` がちゃんと 1 に届くことをテストで固定)。
- `exp`/`ln` は単調なので端点の像で足ります。
- **境界が保証されている範囲の外は拒否**します。`tan` などは剰余の評価を
  書いていないので同様。範囲外のゴールは `Approximate` のままで、それが
  間違いようのない答えです。
- 剰余の上界自体も外側に丸めた区間で計算します。厳密な有理数で
  `|x|⁴¹/41!` を作ると分母があふれるためで、上端を取れば上界のままです。

テストは既知の値だけでなく、**それを定義する恒等式**で交差検査しています
(`sin²+cos²=1`、`ln(exp x)=x`、`exp(a+b)=exp a·exp b`)。どれかの境界が
間違っていればここで壊れます。

途中で見つけたバグ: stdlib の `absR` は `if r < 0.0 then -r else r` と
**単項マイナス**で書かれており、区間モードがそれを扱えていませんでした。
そのため `absR` を使うゴールがすべて浮動小数点扱いに落ちていました。

**残る 8 件**はすべて**反復アルゴリズムにおける区間の依存性**です。
Newton 法の `x - (x³-8)/(3x²)` は同じ変数が複数回現れるので素朴な区間
演算では縮まらず広がります (wrapping effect)。`cbrt8_approx` の囲いは
±1.17e-4 で、要求の 1e-7 に届きません。区間 Newton 法や平均値形式が
要る話で、区間演算のよく知られた限界です。

TCB は 4,424 行 → 4,825 行 (+401)。

### アシュアランスケースを成果物にする

seki のコンセプトは「単位は証明ではなく、主張とその根拠の等級」です。
その形をそのまま成果物にするには 2 つ足りませんでした。

#### 1. `interval lo hi` — 初期状態が「範囲」でも証明できる

区間演算はカーネル内部にしかなく、ソース側に区間の値がありませんでした。
そのため証明できるのは**具体的な数値計算**に限られ、
「初期状態がこの範囲のどこであっても安全」は言えませんでした。

```seki
def startup := interval 15.0 30.0          -- 15〜30°C のどこか

theorem never_overheats_in_10_steps
  : evolve 10 startup <= 25.0 := by eval    -- kernel-checked
theorem settles_near_setpoint
  : absR (evolve 50 startup - 20.0) < 0.1 := by eval
```

範囲を丸ごと運ぶので、これは**範囲内のどの初期状態についても**成り立ちます
— 制御の到達可能性解析そのものです。50 ステップでも 4ms。

境界を割る主張は拒否されます。3 ステップ後の囲いは `[3.187, 4.645]` で、
`< 4.4` は下端では成り立ち上端では成り立たないので証明になりません。
そのとき出るのは「囲いが境界をまたいでいるので、成り立つことも
成り立たないことも示していない」という**証明の失敗**で、
実行時エラーではありません。

#### 2. `seki --audit DIR` — システム全体で 1 枚の論証

1 ファイルの監査は「この定理はどう検証されたか」に答えます。
システムは 1 ファイルではなく、審査員が実際に訊くのは全体についてです。

```
assurance report for examples/assurance
==============================================================================

claims resting on something other than a kernel proof
------------------------------------------------------------------------------
  axiomatic   displayed_temperature_is_trustworthy  (system.seki)
              axiom `sensor_drift_bounded`
              confidence >= 4/5 (from `sensor_drift_bounded` 4/5)

assumed without proof
------------------------------------------------------------------------------
  sensor_drift_bounded    4/5 — 現場ログ6か月 (2026-03〜08), 季節性未補正
  startup_range           19/20 — 運用手順書 SOP-114 (起動前チェック)
```

**カーネルの証明以外に立っている主張から始めます** — それが審査で実際に
見るリストで、良い知らせの下に埋めるのは順序が逆だからです。

- claim は**宣言した場所**で数えます (import で二重計上しない)
- 走らなかったファイルは**所見として列挙**し、黙って飛ばしません
- レポートはバッファして最後に出します (監査対象のプログラムが printf
  するので、間に挟まると成果物になりません)
- 弱い主張か走らないファイルがあれば **exit 1** — ビルドのゲートになります

#### 新しい例: `examples/assurance/`

温度制御ループのアシュアランスケース。1 ディレクトリに 4 種類の根拠が
並びます:

| ファイル | 主張 | 根拠 |
|---|---|---|
| `sensor.seki` | 較正は全入力で仕様内 | 入力集合全体の証明 |
| `controller.seki` | ゲイン範囲全体で有界 | パラメータの箱の証明 |
| `plant.seki` | 起動温度の範囲全体から収束 | 区間演算の到達可能性 |
| `environment.seki` | 環境について仮定すること | 確度と出所つき |
| `system.seki` | 系全体の安全性 | 上の合成 |

14 claims / 5 files / 3 assumptions、うち 11 が kernel 証明、3 が仮定に乗る。


### 区間演算 — 「浮動小数点の主張」の 86% は実は証明できた

前項で 163 件を `approximate` に落としました。正直ですが大雑把です:
それらの主張は実数として**真**であり、足りなかったのは丸めた 1 個の数
ではなく**保証された囲い**で計算する手段でした。

`src/interval.rs` を追加しました。`Interval` は厳密有理数の対 `[lo, hi]`
で真の値を必ず含み、演算は**外側に**丸めるので囲いは広がることしか
ありません。比較は囲い全体が決めたときだけ答え、決まらなければ
「判定なし」— カーネルはそれを決して「成立」とは読みません。

カーネルは `by eval` のゴールを、厳密な有理数 → **区間** → 浮動小数点の
順に判定します。区間で `true` が出れば `sound` です。

```
theorem t : absR (expTaylor 30 1.0 - 2.718281828459045) < 0.0000001 := by eval
  →  kernel-checked from primitives
```

30 項の Taylor 級数を、すべて保証された囲いで計算しています。
`expAccum` は再帰的な seki 関数で、多項式の範囲には全く入りません。

設計上の要点:

- **分母を 10¹² で打ち切る**。厳密有理数の分母は掛け算のたびに積になり、
  数百回で i128 をあふれます。外側に丸めれば 1 演算あたり約 1e-12 広がる
  代わりに数が有界になり、広げるのは常に健全です。
- **`sqrt` は二乗して検査する**。浮動小数点の見積もりから出発しますが、
  各端点は `l² <= x <= h²` を確かめてから採用します。
- **`if` の両枝の包を取る**。条件が囲いで決まらないとき (`absR x` で
  `x` が 0 をまたぐとき、つまり許容誤差チェックのほぼ全部) 両枝を評価して
  包を取ります。真の値はどちらかの枝にあるので健全です。
- **定義はソースから再評価する**。`def pi := 3.141592653589793` は宣言時に
  `f64` になっており、それを読み戻して厳密扱いするのは区間演算が防ごうと
  している混同そのものです。ただし**副作用のある定義は除外** —
  `def counter := mkRef 0` を二度評価すると別のセルになります。
- **丸めた `f64` は区間演算に入れない**。混ざったら判定を返しません。
- **組込みは明示的な表**。`sqrt` / `pow` / `intToReal` だけが囲いを持ち、
  `exp` / `ln` / `sin` / `cos` は失敗します (fail-closed)。

**結果**: `approximate` 163 件 → **22 件**。1041 件の証明は全部そのまま、
失敗ゼロ。残る 22 件は本当に超越関数の囲いが要るものだけです。

この作業中に、区間モードが `def e := parseSym "..."` を stdlib の定数
`e = 2.718…` に解決するバグを見つけました。名前を再定義したときに
古いソースが `def_exprs` に残っていたためです。

### `Real` は ℝ、評価器は `f64` — カーネルが矛盾を承認していた

小数リテラルを 10 進で読むようにした結果、**カーネルが命題とその否定の
両方を `kernel-checked from primitives` と報告する**状態になっていました:

```seki
theorem p    : (0.1 + 0.2) == 0.3 := by algebra   -- 厳密: 3/10 = 3/10
theorem notp : (0.1 + 0.2) != 0.3 := by eval      -- f64: 0.30000000000000004
```

`Real` に 2 つの読みがあったのが原因です。**`Real` は ℝ、`f64` はその
近似**と決めました。カーネルは実数の比較を有理数の範囲内では厳密に決め
(`notp` は拒否)、範囲外 (`sqrt`、関数呼び出しの奥) では評価器の答えを
信頼するが検査したとは言いません。

リテラルを `def` に隠しても抜けられないよう、構文ではなく
**評価器が実数に触れたかどうか** (`EvalCtx::touched_real`) で判定します。

新しい信頼水準 **`Approximate`** (`[approximate — floating point]`) が
これを表します。`Sampled` と同じく「偽を通しうる」側で、`--strict` は
拒否します。

コーパスの 163 件が `sound` から `approximate` に変わりました。中身は
ほぼすべて数値計算の許容範囲チェック (`|integSimpson ... - 1/3| < 1e-6`)
で、**double についての主張**です。以前「primitives から再確立」と
報告していたのは過大な主張でした。

### `by witness` が失敗しても証明書を作っていた

`certify` の `Proof::Witness` 節がタクティクを実行していませんでした。
他の節はすべて `verify_*` を呼んで失敗を伝播させますが、ここだけは
証明書を組み立てるだけだったため、**偽のゴールが「proved [sampled]」に
なっていました**:
...
カーネルは拒否していましたが、拒否された**評価**ステップは
エラーではなく sampled として報告されるため、定理としては登録されて
いました。

### 逆算が非線形の仮定に届くようになった

bound が**積の中**に要るとき、線形の読みでは見えません。
`x >= 0 ⊢ x² <= 4` は `x <= 2` で成り立ちますが、証明書は `(2-x)(2+x)` で、
未知数が生成子に掛かるため線形の問いでなくなります。

そういうゴールでは**証明器に直接訊く**ことにしました。
「`var <= c` でゴールが閉じるか」は `c` について単調なので、倍々のはしごで
境界を挟んでから二分し、読みやすい有理数に丸めて (丸めたものも検証して)
返します。

```
forall x. x >= 0 => x * x <= 4       →  it would hold given `(x <= 2)`
forall x. x >= 0 => x * x * x <= 8   →  it would hold given `(x <= 2)`
forall x. x <= 0 => x * x <= 4       →  it would hold given `(x >= -2)`
forall a b. a,b >= 0, b <= 3 => a*b <= 12  →  it would hold given `(a <= 4)`
```

1 回 1 回が本物の証明試行なので、線形の読みが原理的に無力な**非線形の
ゴールに限って**走らせます。線形のゴールは従来どおり 1 回の解で済みます。
...
足りない仮定の逆算は 1 変数のゴールにしか効いていませんでした。
実用の判断はほぼ多変数なので、これが最大の制約でした。

Farkas を**逆向きに**走らせることで解きます。証明は「非負の重みが
存在するか」を問いますが、逆算は**足りない仮定**を表す列を 1 本足して
同じことを問います。`var <= c` なら生成子は `c - var` で、重み `mu >= 0`
で使われるので、系に `-var` の列が増え、定数項が自由になる
(`mu·c` を吸収する)。答えから `mu` と定数を読めば `c` が直接出ます。

```
forall rev, cost, tax. (rev >= 100) and (tax <= 20) => (rev - cost - tax) >= 50
  it would hold given `(cost <= 30)`
```

**最弱の bound を選ぶ**のが難所でした。`c` は 2 つの未知数の**比**なので、
線形の目的関数では取れません — 定数項を最大化すると
`a >= 0.6 ⊢ 50a >= 40` の答えが `a >= 1000` になります (真ですが無意味)。
Charnes–Cooper 変換 (`y = t·x`, `mu·y = 1`) で比を線形目的に直し、
正しく `a >= 4/5` が出るようにしました。

矛盾する仮定も候補から外します。`a >= 0.6 ⊢ 50a >= 40` は `a <= 0` を
足せば ex falso で「成り立つ」— それをモデルの知見として提示するのは
何も言わないより悪い。

#### 10進リテラルを書いたとおりに読む

`0.6` は「0.6 に最も近い double」ではなく 3/5 として扱います。
以前は二進の正確な値を使っていたので、**`0.6 * 50.0 >= 30.0` が偽**でした
— double については正しく、書かれた主張については間違っています。

副作用として、`0.1 * 0.2 * 0.3` はもう桁あふれしません (3/500)。
「レートは `0.1` ではなく `1.0/10.0` と書け」という回避策は不要になりました。

#### 健全性: 束縛子の無いゴールが Int と誤読されていた
...
これが「✓ proved」になっていました。`forall` が無いとドメインが `Int` に
落ち、整数の離散性 (`p > 0 ⟹ p >= 1`) が実数に誤適用されていたためです。
カーネルは証明書を拒否していた (`[unchecked]`) ので `--strict` は
通していませんが、タクティクが主張すべきではありませんでした。
実数リテラルを含むゴールは `Real` として読みます。

#### 等式の仮定が使えるようになった

「価格は 50」「容量は C」はモデルに頻出しますが、Farkas 証明書は
非負量から作るので、等式の仮定は**捨てられていました**。等式は 2 本の
不等式を含意するので、カーネル側でその 2 本を導出するようにしました
(証明書にどちら向きかを言わせるのではなく)。

#### 監査出力が仮定を報告する

検査済みのものだけを並べる監査は報告として半分です。判断は誰も証明して
いない仮定にも乗っていて、半年後に必要になるのはそちらです。

```
assumed without proof                         confidence and source
------------------------------------------------------------------------
adoption_floor          7/10 — パイロット3社の結果からの外挿 (LLM 要約, 要検証)
discount_rate_range     9/10 — 財務部の想定レンジ (2026Q3 資本コスト表)
```

確度つきの仮定でどの結論も引用していないものには注記が出ます。
「公理の内容を theorem の前提として書き直す」と確度が結論に届かない
— よくあるモデリングの取り違えで、監査がそれを指摘します。

#### 新しいライブラリ: `lib/control/safety.seki`

制御ループの安全包絡線。飽和 (クランプ)、ステップ幅の安全条件、
CFL 条件、ゲインの箱、定常誤差、センサ融合、デッドバンド。
13 件すべて kernel 検証済みで、範囲を 1 つ緩めた版はすべて拒否されます。

#### 新しい例: `examples/46_decision_audit.seki`

意思決定の監査証跡。パラメータを未定義の名前として置き、確度と出所つきの
仮定から結論を導く。最悪ケースの結論は確度 1/5 (Fréchet 下界) と出る —
それが「この判断がどれだけ支えられているか」の正直な数字です。


### ℝ を公理化し、解析を演繹で建てる

`lib/analysis/` はこれまで**数値ライブラリ**でした。ε-δ の定義は
`lib/analysis/limit.seki` に書かれてはいましたが、そこには
「`theorem` の証明には使わないこと」という注意書きが付いていました。
存在命題を**導入**する手段が無く、δ を差し出せなかったからです。

3 つの規則を足してその穴を埋めました。いずれも証明書の形を持つので、
`seki --audit` が「これは検査された」と言えます。

| 規則 | 何ができるようになったか |
|---|---|
| `by witness v := <項>` / `Cert::Witness` | `exists` の導入。ε-δ が書けるだけでなく**証明できる**ようになった |
| `Cert::Antisymmetry` | `<=` と `>=` の両方から `==`。極限・上限の一意性 |
| `Cert::CancelPositive` | 正の量で割る。縮小写像の議論はすべてここで止まっていた |

`by witness` の項は構文的に全域性を検査します — `eps / 2.0` は通り、
`1.0 / eps` は通りません。`eps != 0` が仮定にあっても、それを読むのは
推論であって所属判定ではないからです。

新しいライブラリ:

- **`lib/analysis/axioms.seki`** — ℝ を完備順序体として公理化。
  順序体の公理は**公理ではなく定理**として証明する (`by algebra` が
  決定手続きで落とすので、何を仮定しているかを読む人に見せられる)。
  本当の仮定は 2 つだけ: **完備性** (上限公理) と**アルキメデス性**。
- **`lib/analysis/continuity.seki`** — 連続性・Lipschitz 連続・極限・
  微分を、上の定義から `by witness` で演繹する。29 件すべて kernel 検証済み。
- **`lib/analysis/ode.seki`** — Picard–Lindelöf の**演繹できる半分**。
  一意性 (縮小写像の不動点は高々ひとつ) は証明し、存在 (Banach) は
  関数空間の完備性が要るので公理に置く。Euler 法の 1 段の安定性と
  縮小条件も証明済み。

微分は差商 `(f(x0+h) - f(x0))/h` ではなく、**割らない**形で定義しました:

```
f'(x0) = L  ⟺  ∀ε>0 ∃δ>0. |h| < δ ⟹ |f(x0+h) - f(x0) - L·h| <= ε|h|
```

教科書の定義と同値で、`h != 0` の場合分けが要らず、`by algebra` の
多項式の範囲に収まり、そのまま多変数と ODE に伸びます。差商のままだと
`2h/h == 2` の約分が必要で、それは証明できても**証明項が付きません**。
...
Farkas 証明書は「仮定を非負の重みで**足す**」ことしかできず、
`0 <= a <= 1 ⊢ a² <= 1` に届きませんでした。`a` と `1-a` をどう足しても
`1 - a²` にはならないからです。

届く道具は**仮定の積**です。`p >= 0` かつ `q >= 0` なら `pq >= 0` は何も
追加で仮定せずに成り立つので、積もまた足してよい非負量です:

```
1 - a² = (1 - a) + a·(1 - a)
```

- `kernel::Generator` — 証明書の構成要素を「仮定の積」に一般化。
  単一因子なら従来の Farkas 項です。kernel は各因子がゴールの仮定で
  あることを確認し、掛けて足して比べるだけ。**積が strictly positive な
  のは全因子が strict なときだけ**という条件も検査します。
- 探索は仮定とその対ごとの積を生成子とし、小さい部分集合から厳密に解きます。
  探索は難しく検査は易しい、という設計はそのままです。
- タクティク側 (`prove_algebra_rel`) にも接続したので、`by algebra` 自体が
  非線形を扱えるようになりました。

通るようになったもの:

```seki
theorem square_bounded : forall a in Real, (0.0 <= a) and (a <= 1.0) => (a*a) <= 1.0
theorem box_bound      : 0<=x<=2, 0<=y<=3 => x*y <= 6
theorem am_gm_two      : a,b >= 0 => (a+b)² >= 4ab
```

**非線形の refinement 型**も通ります:

```seki
def Unit01 := {x in Real | (0.0 <= x) and (x <= 1.0)}
def square   : Unit01 -> Unit01          := \x -> x * x        -- 証明される
def multiply : Unit01 -> Unit01 -> Unit01 := \x y -> x * y     -- 証明される
def doubled  : Unit01 -> Unit01          := \x -> x * 2.0      -- [sampled]
```

届かないもの: **次数3以上** (`a³ <= 1`) は3つ組の積が必要で、現在の探索は
対までです。生成子の数が組合せ的に増えるため、部分集合探索ではなく
厳密有理数の単体法 (phase-1 simplex) に置き換えるのが次の一手です。

- `examples/45_nonlinear.seki`
- 偽造テストを2件追加 (ゴールが仮定していない因子を使う積、
  strict でない因子を含む積を strictly positive と主張する証明書)
