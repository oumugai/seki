...
という自己紹介で始まり、特徴が 20 個ほど並ぶ形です。実装がそこから動いたので、
書き直しました。

いまの seki の形は「**単位は証明ではなく、主張とその根拠の等級**」です。
計算を走らせることとその計算について主張することが同じ行為で、処理系は主張ごと
にどの種類の根拠が得られたかを報告する。成果物は `--audit` の表であって、
`theorem` はその行を生む手段。

新しい構成:

1. 冒頭にコンセプトと、**テストが緑のままバグが通る例**
2. `--audit` と 5 段の信頼水準 — 低い 2 段は「偽を通しうる」と明記
3. 「この集合のすべての値について」を言う **3 つの方法**と、どれが担ったかの記録
4. 探索と検査の分離 (TCB を**予算**として扱う話)
5. 仮定の追跡 (Fréchet 下界)、失敗が情報を返す話 (逆算)
6. **限界 — 名前と理由つきで**
7. 言語としての seki、何に向くか・向かないか
8. **この設計の弱点** — 5 段の格子は正直さの上にしか成り立たない

陳腐化していた数字も直しました:

| | 旧 | 実測 |
|---|---|---|
| TCB | 約 2,600 行 | **4,896 行** (全体の 17%) |
| 主張 | 968 中 925 が kernel 検証 | **1,214 中 1,148** (sound) |
| 戦術 | 12 種類 | **15 種類** |
| `--proof` の出力例 | 古い | 実出力に差し替え |

README に載せたコード片と出力は**すべて実際に走らせて確かめて**あります
(`docs/` の記述が実装より古くなる問題が何度も起きているので — seki-dev skill 参照)。

区間・`--audit DIR`・`by witness`・解析ライブラリ・制御ライブラリ・
`examples/assurance/`・`examples/services/` は README に一度も載っていませんでした。


### システム開発の例 — 書いてみて見つかった 3 つの穴

`examples/services/` に、普通のバックエンド開発で書くものを 5 つ置きました。
いずれも「テストでは書けないが、コードを書き換えなくても言えること」です。
...
| `sla.seki` | 合成可用性が SLO を満たす | 確度と出所つきの仮定 |
| `capacity.seki` | ピーク時の容量が足りる | パラメータの箱、足りない仮定の逆算 |

18 claims / 5 files、うち 17 が kernel 証明。書く過程で 3 つ穴が出ました。

#### 整数の離散性が証明書側に無かった

`i < r` は `Nat` 上では `r >= i + 1` です。探索側はこれを使っていましたが
**証明書側が使っていなかった**ため、`if i < r` の else 枝 —
`i < r` と `i >= r` を両方仮定する矛盾した枝 — から出る等式に証明書が
付きませんでした。条件が排除する `match` の腕はすべてこの形なので、
影響は広い範囲に及びます。

カーネルが自分で導出するようにしました (`add_integer_consequences`)。
実数上では導けないので、同じ形は `unchecked` のままです — そこが
正しい区別です。

#### 等式のゴールに `if` があると場合分けしなかった

値を返す `match` はこの形に脱糖されます。等式の経路は場合分けを試す前に
諦めていました。

#### 逆算が「強い方向」を探していなかった

`perNode >= 0` は `perNode · nodes >= rps` について何も言いません。
必要な bound は 0 より**強い**側にあるのに、探索は 0 から弱い方向にしか
歩いていませんでした。二段階にして、まず強い方向で成り立つ点を見つけ、
そこから弱い方向に戻って境界を挟むようにしました。
